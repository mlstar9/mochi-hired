...
export const WorkflowDemi: React.FC<{gawxFilter?: boolean; transparent?: boolean}> = ({gawxFilter = true, transparent = false}) => {
  ...
  const demiX = 720;
  const demiY = 260;
  const demiFrame = 48; // 2s into comp

  // Glitch: Demi → Semi after 2s hold (48 frames at 24fps)
  const glitchStart = demiFrame + 48;
    ...
    <AbsoluteFill style={{backgroundColor: transparent ? 'transparent' : '#111111'}}>
      ...
      {row.map((r, i) => (
        <DoodleArrow key={`a-${i}`}
          ...
          startFrame={24 + i * 2} seed={610 + i * 10} strokeWidth={2.5} />
      ))}
